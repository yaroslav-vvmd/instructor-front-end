...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const allChildrenInvisible = Array.from(instructorSocial.children).every(
  ...
  function appendReadMoreButton(truncatedText) {
    ...
    if (lastElement && lastElement.nodeType === Node.ELEMENT_NODE) {
      ...
      wrapper.innerHTML =
        ...
        `<span id="dots">...</span><span id="readMoreBtn" class="instructor_read-more">Показати більше</span>`;
    ...
    readMoreBtn.addEventListener("click", () => {
      if (!isExpanded) {
        instructorText.innerHTML =
          originalText +
          `<span class="instructor_read-more" id="readMoreBtn" style="margin-left: 0;">Показати менше</span>`;
        isExpanded = true;
      } else {
        instructorText.innerHTML = finalTruncatedText;
        isExpanded = false;
    ...
    function attachListener() {
      ...
      newReadMoreBtn.addEventListener("click", () => {
        if (!isExpanded) {
          instructorText.innerHTML =
            originalText +
            `<span id="readMoreBtn" class="instructor_read-more" style="margin-left: 0;">Показати менше</span>`;
          isExpanded = true;
        } else {
          instructorText.innerHTML = finalTruncatedText;
          isExpanded = false;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (slug) {
    fetch(
      `https://instructor-backend.vercel.app/api/cms-item/${type}/${slug}`,
      {
        method: "GET",
        headers: {
        ...
        if (data.item.id) {
          ...
          form.addEventListener("submit", (event) => {
            ...
            const overallRating = form.querySelector(
              'input[name="main-rating"]:checked'
            ).value;
            const qualityOfEducation = form.querySelector(
              'input[name="quality-of-education"]:checked'
            ).value;
            const instructorAttitude = form.querySelector(
              'input[name="attitude-of-the-instructor"]:checked'
            ).value;
            const carCondition = form.querySelector(
              'input[name="car-condition"]:checked'
            ).value;
            const reviewText = form.querySelector(
  ...
  function fetchTestimonials(cmsItemId, type) {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        const testimonialsAmount = document.querySelector(
          ".testimonials_amount"
        );
        const rateElement = document.querySelector(
          ".instructor-content_head-rate"
        );
        const totalElement = document.querySelector(
          ".instructor-content_head-number"
        );
        const empty1 = document.querySelectorAll(
          ".instructor-content_head-empty"
        )[0];
        const empty2 = document.querySelectorAll(
          ".instructor-content_head-empty"
        )[1];
        ...
        const totalRatings = testimonials.reduce(
          (sum, testimonial) => sum + parseFloat(testimonial.overallRating),
          0
        );
        const averageRating =
        ...
        document.getElementById("averageQualityOfEducationSVG").innerHTML =
          data.averageQualityOfEducationSVG;
        document.getElementById("averageInstructorAttitudeSVG").innerHTML =
          data.averageInstructorAttitudeSVG;
        document.getElementById("averageCarConditionSVG").innerHTML =
          data.averageCarConditionSVG;
        ...
        if (testimonials.length <= testimonialsToShow) {
          document.querySelector(".testimonials_load-more").style.display =
            "none";
        } else {
          document.querySelector(".testimonials_load-more").style.display =
            "flex";
        ...
        testimonials.forEach((testimonial) => {
          const likeIcon = document.querySelector(
            `.testimonial_helpful-item.like[data-testimonial-id="${testimonial.testimonialId}"]`
          );
          const dislikeIcon = document.querySelector(
            `.testimonial_helpful-item.dislike[data-testimonial-id="${testimonial.testimonialId}"]`
          );

          if (testimonial.likedBy && testimonial.likedBy.includes(userId)) {
            if (likeIcon) {
              likeIcon
                .closest(".testimonial_helpful-item")
                .classList.add("active");
            }
          }
          if (
            testimonial.dislikedBy &&
            testimonial.dislikedBy.includes(userId)
          ) {
            if (dislikeIcon) {
              dislikeIcon
                .closest(".testimonial_helpful-item")
    ...
    visibleTestimonials.forEach((testimonial) => {
      const testimonialHTML = `
                                <div class="testimonials_slide swiper-slide">
                                    <div class="testimonials_testimonial">
                                        <div class="testimonial_head">
                                         <div class="testimonial__name">${testimonial.firstName &&
          ...
          : "Anonymous"
                                        ...
                                        <p class="testimonial_text">${testimonial.text
        }</p>
  ...
  document
    .querySelector(".testimonials_list-inner")
    .addEventListener("click", function (event) {
      if (event.target.closest(".testimonials_show-more-text")) {
        ...
        const testimonialId = event.target.closest(
          ".testimonial_helpful-item.like"
        ).dataset.testimonialId;
        const reviewerId = event.target.closest(
          ".testimonial_helpful-item.like"
        ).dataset.userId;
        likeTestimonial(testimonialId, reviewerId);
      } else if (event.target.closest(".testimonial_helpful-item.dislike")) {
        const testimonialId = event.target.closest(
          ".testimonial_helpful-item.dislike"
        ).dataset.testimonialId;
        const reviewerId = event.target.closest(
          ".testimonial_helpful-item.dislike"
        ).dataset.userId;
  ...
  // Create a map to track processing state for each testimonial
  ...
  function likeTestimonial(testimonialId, reviewerId) {
    ...
    // Check if the testimonial is already being processed
    ...
    fetch("https://instructor-backend.vercel.app/api/testimonials/like", {
      ...
      .then((data) => {
        const likeIcon = document.querySelector(
          `.testimonial_helpful-item.like[data-testimonial-id="${testimonialId}"]`
        );
        const dislikeIcon = document.querySelector(
          `.testimonial_helpful-item.dislike[data-testimonial-id="${testimonialId}"]`
        );
        const likeCountElement = likeIcon.querySelector(
          ".testimonial_like-count"
        );
        const dislikeCountElement = dislikeIcon.querySelector(
          ".testimonial_dislike-count"
        );

        const currentLikeCount = parseInt(likeCountElement.textContent) || 0;
        const currentDislikeCount =
          parseInt(dislikeCountElement.textContent) || 0;
        ...
        if (data.message.includes("liked successfully")) {
          ...
          if (dislikeIcon.classList.contains("active")) {
            ...
            dislikeCountElement.textContent = Math.max(
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function hideSectionIfEmpty() {
    ...
    const practicalList = document.querySelectorAll(
      "#practical-list .swiper-slide"
    );
  ...
  const tabButtons = document.querySelectorAll(
    ".instructor-in-city_head-switch"
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mobileInformationOverlay = document.querySelector(
    "#mobile-information-overlay"
  );
  const mobileInformationClose = document.querySelector(
    "#mobile-information-close"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  phoneLinks.forEach((phoneLink) => {
    ...
    if (window.innerWidth > 767) {
      ...
      phoneLink.querySelector(".truncated-number").textContent =
        truncatedNumber;
...
document.addEventListener("DOMContentLoaded", function () {
  const items = document.querySelectorAll(
    ".instructor_images-list .instructor_images"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  truncatedElement.forEach((el) => {
    ...
    if (
      originalTextContent.includes("http://") ||
      originalTextContent.includes("https://")
    ) {
  ...
  serviceItems.forEach((item) => {
    ...
    if (phone) {
      phone.addEventListener("click", () => {
        ...
        window.handleSubscription(slug, false);
      });
    }
